...
class G2p(object):
    def __init__(self):
        super().__init__()
        self.graphemes = ["<pad>", "<unk>", "</s>"] + list("abcdefghijklmnopqrstuvwxyz") # 29个字符
        ...
        self.cmu = cmudict.dict() #是一个音速字典, 给一个单词返回他的音素也就是音标的列表.
        self.load_variables()
    ...
    def predict(self, word):
        # encoder=========作者说了,为了脱离tensorflow的依赖,他用numpy实现的神经网络.
        ...
        # decoder
        dec = np.take(self.dec_emb, [2], axis=0)  # 2: <s>  计算 <s>的特征向量.
        h = last_hidden

        preds = []
        for i in range(20): #注意这里面是seq2seq模型.  也就是经典的翻译模型, 喂入之前的一个字符,吐出一个字符. 同时也更新里面的h跟 dec在每次迭代.
        ...
        preds = [self.idx2p.get(idx, "<unk>") for idx in preds] # 从编码转化为音素
        return preds
    ...
    def __call__(self, text):
        ...
        # tokenization
        words = word_tokenize(text) #调用nltk的词性分析.
        tokens = pos_tag(words)  # tuples of (word, tag)

        # steps
        prons = []
        for word, pos in tokens:
            if re.search("[a-z]", word) is None: #如果是非英语字符直接独立成一个pron
                pron = [word]
            ...
            else: # predict for oov  #=======如果上面都没有, 那么就用神经网络猜一个, 很简单的任务是一个74分类任务.
...
if __name__ == '__main__':
    texts = ["aaaaaaaaaaaaaaaa, I have $250 in my pocket.", # number -> spell-out
